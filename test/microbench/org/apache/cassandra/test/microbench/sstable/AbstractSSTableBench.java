...
public abstract class AbstractSSTableBench
{
    ...
    {
        return DatabaseDescriptor.getSSTableFormats().get(formatName);
    }
    ...
    public SSTableWriter createWriter(ColumnFamilyStore table, SSTableFormat<?, ?> format, LifecycleTransaction txn) throws Exception
    {
        ...
        Descriptor desc = table.newSSTableDescriptor(new File(tableDir), format);

        return format.getWriterFactory().builder(desc)
                     .setTableMetadataRef(table.metadata)
                     .setSerializationHeader(new SerializationHeader(true, table.metadata(), table.metadata().regularAndStaticColumns(), EncodingStats.NO_STATS))
                     .setSecondaryIndexGroups(table.indexManager.listIndexGroups())
                     .addDefaultComponents(table.indexManager.listIndexGroups())
                     .setMetadataCollector(new MetadataCollector(table.metadata().comparator))
                     .build(txn, table);
    }
