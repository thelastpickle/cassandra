...
[ "x${rpm_dir}" != "x" ] || rpm_dir="$(mktemp -d /tmp/cassandra-package-rpm.XXXXXXXXXX)"
...
if [ "$1" == "-h" ]; then
   echo "$0 [-h] [<dist_type> [<java version>]]"
...
[ -f "${cassandra_dir}/.build/docker/${dist_name}-image.docker" ] || { echo >&2 "${cassandra_dir}/.build/docker/${dist_name}-image.docker must exist"; exit 1; }
[ -f "${cassandra_dir}/.build/docker/build-rpms.sh" ] || { echo >&2 "${cassandra_dir}/.build/docker/build-rpms.sh must exist"; exit 1; }
...
docker image prune --all --force --filter label=org.cassandra.buildenv=${dist_name} --filter "until=24h" || true
...
image_tag="$(md5sum docker/${dist_name}-image.docker | cut -d' ' -f1)"

# Create build images containing the build tool-chain, Java and an Apache Cassandra git working directory, with retry
until docker build --build-arg UID_ARG=`id -u` --build-arg GID_ARG=`id -g` -t cassandra-packaging-${dist_name}:${image_tag} -f docker/${dist_name}-image.docker docker/  ; do echo "docker build failed… trying again in 10s… " ; sleep 10 ; done

# Run build script through docker (specify branch, tag, or sha)
docker run --rm -v "${cassandra_dir}:/home/build/cassandra" -v "${rpm_dir}":/dist -v ~/.m2/repository/:/home/build/.m2/repository/ cassandra-packaging-${dist_name}:${image_tag} /home/build/build-rpms.sh ${java_version} ${rpm_dist}

mkdir -p ${cassandra_dir}/build/packages/rpm
cp -r ${rpm_dir}/* ${cassandra_dir}/build/packages/rpm
popd
