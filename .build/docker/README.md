...
Once triggered, a new branch or an unavailable previous commit selects all tracked Dockerfiles.  A force push can make the previous commit unavailable even with a full-history checkout.  Apache-only publication runs still check the registries before building.
...
For Apache-only publication, the workflow skips a tag only when both registries contain both required Linux platforms.  A missing tag, incomplete manifest, or missing mirror triggers a build.  Authentication, network, and other unexpected registry errors fail the run instead of triggering a rebuild.
...
This checks the Dockerfiles and runner environment without pushing code to `apache/cassandra`.  It does not test registry writes or duplicate suppression under real GitHub scheduling.  GHCR publication can test registry writes from the fork.

## GHCR publication

Manual runs can publish to the GitHub Container Registry (GHCR) from forks or upstream.  Enable `publish_ghcr` and leave `publish` disabled to use GHCR without Docker Hub or JFrog credentials.  Leave `image` empty to build all three images for both `linux/amd64` and `linux/arm64`.

```sh
gh workflow run docker-images.yaml --repo thelastpickle/cassandra \
  --ref YOUR_BRANCH -f publish=false -f publish_ghcr=true
```

The workflow must exist on the repository's default branch and the selected branch.  Replace `--ref` with the branch to publish.

Images use `ghcr.io/<repository-owner>/cassandra-<dockerfile basename>:<dockerfile md5sum>`, with the owner converted to lowercase.  Each tag contains both architectures, provenance, and SBOM attestations.  The workflow verifies both architectures after publishing each tag.  GHCR runs build every selected image, even when its Apache registry tags already exist.

Authentication uses the workflow's `GITHUB_TOKEN` with `packages: write`; no additional secret is required.  Organisation policy must permit package creation.  New packages are private by default.  Change each package's visibility to public for anonymous pulls, or grant readers access to private packages.  Existing packages must grant the workflow repository write access.

For example, pull the Debian image using its Dockerfile tag:

```sh
docker pull ghcr.io/thelastpickle/cassandra-debian-build:<dockerfile-md5>
```

Docker selects the host architecture.  Use `--platform linux/amd64` or `--platform linux/arm64` to select one explicitly.  The other image basenames are `almalinux-build` and `ubuntu-test`.
...
| --- | --- | --- |
| `publish` | `false` | Set `true` to publish to both Apache registries; upstream release branches only. |
| `publish_ghcr` | `false` | Set `true` to publish to GHCR under the repository owner, including on forks. |
