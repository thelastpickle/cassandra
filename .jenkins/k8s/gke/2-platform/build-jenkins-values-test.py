...
class PipelineCapacityTest(unittest.TestCase):
    ...
    def test_site_https_url_survives_generation(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp) / "base.yaml"
            out = Path(tmp) / "values.yaml"
            base.write_text(yaml.safe_dump({"controller": {"jenkinsUrl": "https://ci.example.org"}}))
            result = subprocess.run([sys.executable, str(HERE / "build-jenkins-values.py"), "--base", str(base),
                                     "--out", str(out), "--hostname", "ci.example.org"],
                                    check=True, capture_output=True, text=True)
            self.assertEqual("https://ci.example.org", yaml.safe_load(out.read_text())["controller"]["jenkinsUrl"])
            self.assertIn("Jenkins reports itself as https://ci.example.org", result.stdout)
            self.assertNotIn("TLS: none", result.stdout)
