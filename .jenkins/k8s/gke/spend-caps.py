...
DEFAULT_PRICE_PER_VCPU_HOUR = 0.04
...
# The guard evaluates caps every {interval} minutes. A null cap disables that window.
"""
...
def describe(figures: dict, caps: dict, out: Path) -> str:
    ...
    lines.append(f"    the regional cluster's management fee, the forwarding rule, the controller's disk, and")
    ...
    if figures["burn_per_hour"] is not None:
        ...
        lines.append("  How many agent nodes this project allows is not known here, so no figure below")
...
def prompt(figures: dict, existing: dict, attempts: int = 3) -> dict:
    ...
    print("cap for that window. A project quota limits how many nodes may exist at once, and nothing here")
    print("limits money, which is what these three answers are for. Every figure is US dollars, and every")
    print("window is UTC: the day starts at midnight, the week on Monday, the month on the 1st.")
    print()
...
def main() -> int:
    ...
    parser.add_argument("--max-agent-nodes", type=int,
                        help="How many agent nodes this project allows, for the arithmetic. Defaults to"
                             " $GKE_MAX_AGENT_NODES, which `make quota` writes.")
    parser.add_argument("--price-per-vcpu-hour", type=float,
                        default=float(os.environ.get("GKE_SPEND_PRICE_PER_VCPU_HOUR",
                                                     DEFAULT_PRICE_PER_VCPU_HOUR)))
    parser.add_argument("--fixed-usd-per-day", type=float,
                        default=float(os.environ.get("GKE_SPEND_FIXED_USD_PER_DAY",
    ...
    parser.add_argument("--interval-minutes", type=int,
                        default=int(os.environ.get("GKE_SPEND_GUARD_INTERVAL_MINUTES", "5")),
    ...
    if nodes is None and os.environ.get("GKE_MAX_AGENT_NODES"):
        nodes = int(os.environ["GKE_MAX_AGENT_NODES"])
    ...
    if args.check:
        ...
        if existing["email"]:
            ...
            print("  No address is set, so a cap being met is published to Pub/Sub and to Cloud Monitoring only.")
        return 0
