...
output "kubernetes_version" {
  description = "The control plane's Kubernetes version, as EKS reports it rather than as this configuration asked for it. 2-platform reads it to choose a matching cluster autoscaler image: the autoscaler is versioned against the Kubernetes minor it schedules for, and a mismatch shows up as pods the autoscaler declines to act on rather than as an error."
...
output "tls_certificate_arn" {
  description = "ACM certificate for var.jenkins_hostname, or empty when there is none."
  value       = join("", aws_acm_certificate.jenkins[*].arn)
}

output "dns_nameservers" {
  description = "The nameservers of the hosted zone named by var.dns_hosted_zone_id."
...
output "agent_node_ceiling" {
  precondition {
    ...
    error_message = "The minimum agent pool sizes exceed the CPU, storage or per-zone subnet budget. The small pool needs small_workers_per_build + 1 slots; raise capacity or reduce other pools."
  }
