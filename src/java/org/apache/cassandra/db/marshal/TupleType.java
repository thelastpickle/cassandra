...
public class TupleType extends AbstractType<ByteBuffer>
{
    ...
    @VisibleForTesting
    public TupleType(Iterable<AbstractType<?>> subTypes, boolean freezeInner, boolean isMultiCell)
    {
        this(ImmutableList.copyOf(freezeInner ? transform(subTypes, AbstractType::freeze) : subTypes), isMultiCell);
    }

    private TupleType(ImmutableList<AbstractType<?>> subTypes, boolean isMultiCell)
    {
    ...
    {
        return new TupleType(subTypes().stream().map(t -> t.overrideKeyspace(overrideKeyspace)).collect(ImmutableList.toImmutableList()), true, isMultiCell());
    }
    ...
    {
        return new TupleType(parser.getTypeParameters(), true, false);
    }
    ...
    public TupleType withUpdatedUserType(UserType udt)
    {
        return referencesUserType(udt.name)
             ? new TupleType(transform(subTypes, t -> t.withUpdatedUserType(udt)))
             : this;
    }
    ...
    {
        return new TupleType(transform(subTypes, AbstractType::expandUserTypes));
    }
