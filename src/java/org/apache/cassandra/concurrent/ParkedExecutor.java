    ...
    public static ShutdownableExecutor createParkedExecutor(String name, int threadCount)
    {
        ...
        if (threadCount == 1)
        {
            return ParkedThreadsMonitor.IS_ENABLED ?
                   ...
                   new FallbackExecutor(ExecutorFactory.Global.executorFactory().sequential(name));
        }
        return ParkedThreadsMonitor.IS_ENABLED ?
               new ParkedMultiThreadedExecutor(name, threadCount) :
               new FallbackExecutor(ExecutorFactory.Global.executorFactory().pooled(name, threadCount));
    }
